...
class DoctorTelemedicineScreen extends StatefulWidget {
  const DoctorTelemedicineScreen({
    super.key,
...
class _DoctorTelemedicineScreenState extends State<DoctorTelemedicineScreen> {
  ...
  @override
  void didUpdateWidget(covariant DoctorTelemedicineScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    final next = widget.initialSessionId;
    if (next != null &&
        next.isNotEmpty &&
        next != oldWidget.initialSessionId &&
        _activeSession?.id != next) {
      _bootstrap();
    }
  }
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      final sessions = await widget.dashboardService.getTelemedicineSessions(
  ...
  Future<void> _send() async {
    ...
    try {
      final message = await widget.dashboardService.sendTelemedicineMessage(
        token: widget.session.token,
        sessionId: active.id,
        content: text,
        files: attachments,
    ...
    if (widget.session.userType.toLowerCase() != 'doctor') {
      return const Center(
        child: Padding(
          ...
          child: Text(
            'Telemedicine is currently available for Doctor accounts.',
          ),
        ),
      );
    }
                  ...
                  items: _sessions
                      .map(
                        (session) => DropdownMenuItem<String>(
                          value: session.id,
                          child: Text(
                            '${session.patientName} • ${session.appointmentDate} ${session.appointmentSlot}',
